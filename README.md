...
`stegoX` is a small learning-oriented text steganography toolkit. It reimplements the reference project in `reference_project/SteganographyLibrary` with the same main ideas:

- edit-based text steganography with a masked language model
- bins/LSTM-style generation with a causal language model
...
- `lstm`: Shuffles the vocabulary into deterministic bins and chooses the highest-probability token inside the bin indicated by each bit block.
